export function UtxoListComponent(container) {
  ...
  function getScriptType(utxoData) {
    ...
    if (spendType.includes('seed') || spendType.includes('regular')) return 'p2wpkh';
    if (spendType.includes('swap') || spendType.includes('contract') || spendType.includes('fidelity')) return 'p2wsh';
  ...
  function getScriptTypeColor(scriptType) {
    switch (scriptType) {
      case 'p2wpkh': return 'green';
      case 'p2wsh': return 'blue';
      case 'p2tr': return 'purple';
      default: return 'gray';
    }
  }
  ...
  function calculateStats() {
    ...
    const totalValue = filteredUtxos.reduce((sum, utxo) => sum + utxo.utxo.amount, 0);
    const confirmed = filteredUtxos.filter(utxo => utxo.utxo.confirmations > 0).length;
    const unconfirmed = totalUtxos - confirmed;

    // Count by script type (from all UTXOs, not filtered)
    const p2wpkhCount = allUtxos.filter(u => getScriptType(u) === 'p2wpkh').length;
    const p2wshCount = allUtxos.filter(u => getScriptType(u) === 'p2wsh').length;
    const p2trCount = allUtxos.filter(u => getScriptType(u) === 'p2tr').length;

    return { totalUtxos, totalValue, confirmed, unconfirmed, p2wpkhCount, p2wshCount, p2trCount };
  }
  ...
  function applyFilter(filterType) {
    ...
    if (filterType === 'all') {
      ...
      filteredUtxos = allUtxos.filter(utxo => getScriptType(utxo) === filterType);
    }
  ...
  function updateSelectionUI() {
    ...
    if (selectAllCheckbox) {
      selectAllCheckbox.checked = selectedUtxos.length === filteredUtxos.length && filteredUtxos.length > 0;
      selectAllCheckbox.indeterminate = selectedUtxos.length > 0 && selectedUtxos.length < filteredUtxos.length;
    }
    ...
    if (selectedUtxos.length > 0) {
      ...
      const totalSelected = selectedUtxos.reduce((sum, idx) => sum + filteredUtxos[idx].utxo.amount, 0);
  ...
  async function loadUtxos() {
    try {
      ...
      if (tableBody) {
        tableBody.innerHTML = '<tr><td colspan="7" class="text-center py-8 text-red-400">Failed to load UTXOs. Please try refreshing.</td></tr>';
      }
    }
  }
  ...
  function updateStatsDisplay() {
    ...
    if (p2wpkhBtn) {
      p2wpkhBtn.querySelector('.filter-count').textContent = `(${stats.p2wpkhCount})`;
    }
    if (p2wshBtn) {
      p2wshBtn.querySelector('.filter-count').textContent = `(${stats.p2wshCount})`;
    }
    if (p2trBtn) {
      p2trBtn.querySelector('.filter-count').textContent = `(${stats.p2trCount})`;
    }
  }
  ...
  function updateUtxoTable() {
    ...
    if (filteredUtxos.length === 0) {
      const message = activeTypeFilter === 'all' 
        ? 'No UTXOs found' 
    ...
    tableBody.innerHTML = filteredUtxos.map((utxoData, index) => {
      const utxo = utxoData.utxo;
      const spendInfo = utxoData.spendInfo;
        ...
        <div class="bg-[#1a2332] rounded-lg p-4 mb-6">
            <div class="flex items-center justify-between">
                ...
                <div class="text-xs text-gray-500">
                    <span class="text-green-400">P2WPKH</span> = Native SegWit | 
  ...
  content.querySelector('#filter-all').addEventListener('click', () => applyFilter('all'));
  content.querySelector('#filter-p2wpkh').addEventListener('click', () => applyFilter('p2wpkh'));
  content.querySelector('#filter-p2wsh').addEventListener('click', () => applyFilter('p2wsh'));
  content.querySelector('#filter-p2tr').addEventListener('click', () => applyFilter('p2tr'));
