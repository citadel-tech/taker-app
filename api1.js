...
function getCurrentWalletName() {
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      const walletMatch = configContent.match(/wallet_file_name\s*=\s*"([^"]+)"/);
      if (walletMatch) {
...
function saveSwapReport(swapId, swapData) {
  try {
    ...
    const reportsDir = path.join(api1State.DATA_DIR, 'swap_reports', walletName);
...
function registerTakerHandlers() {
  ...
  ipcMain.handle('taker:initialize', async (event, config) => {
    try {
      ...
      console.log(`📦 [API] Initializing taker with ${protocolName} protocol...`);
      ...
      console.log('🔧 Initializing Taker with:', { walletName, zmqAddr, protocol });
      ...
      if (!TakerClass) {
        ...
        return { success: false, error: `${missing} class not found in coinswap-napi. Please rebuild the native module.` };
      }
      ...
      console.log(`✅ ${protocolName} Taker initialized with wallet:`, walletName);
      ...
      return { success: true, message: `${protocolName} Taker initialized and ready`, protocol };
    } catch (error) {
      console.error('❌ Initialization failed:', error);

      if (error.message.includes('decrypting wallet') || error.message.includes('wrong passphrase')) {
  ...
  ipcMain.handle('taker:getWalletInfo', async () => {
    try {
      const walletName = api1State.currentWalletName || api1State.DEFAULT_WALLET_NAME;
  ...
  ipcMain.handle('taker:getTransactions', async (event, { count = 10, skip = 0 }) => {
    try {
      ...
      const transactions = api1State.takerInstance.getTransactions(parseInt(count), parseInt(skip));
  ...
  // Restore wallet
  ipcMain.handle('taker:restore', async (event, { backupFilePath, password, walletName }) => {
    try {
      if (!api1State.coinswapNapi) {
        await initNAPI();
        if (!api1State.coinswapNapi) {
      ...
      console.log(`📁 Restoring to: ${api1State.DATA_DIR}/wallets/${restoredWalletName}`);
  ...
  ipcMain.handle('taker:isWalletEncrypted', async (event, walletPath, walletName) => {
    try {
      if (!walletPath) {
        const name = walletName || api1State.currentWalletName || api1State.DEFAULT_WALLET_NAME;
      ...
      const isEncrypted = api1State.coinswapNapi.Taker.isWalletEncrypted(walletPath);
      return isEncrypted;
    } catch (error) {
  ...
  ipcMain.handle('taker:syncOfferbook', async () => {
    try {
      ...
      const offerbookPath = path.join(api1State.storedTakerConfig.dataDir, 'offerbook.json');
      if (fs.existsSync(offerbookPath)) {
        fs.unlinkSync(offerbookPath);
      }
      ...
      api1State.activeSyncs.set(syncId, { status: 'starting', startedAt: Date.now() });
      ...
      if (fs.existsSync(offerbookPath)) {
        ...
        if (offerbook.all_makers) {
          const badMakerAddresses = new Set(
            (offerbook.bad_makers || []).map(m => `${m.address.onion_addr}:${m.address.port}`)
          );
          ...
          return { success: true, offerbook: transformedOfferbook, cached: true };
        }
      }
...
function registerCoinswapHandlers() {
  // Start coinswap
  ipcMain.handle('coinswap:start', async (event, { amount, makerCount, outpoints, password }) => {
    try {
      if (!api1State.takerInstance) {
        return { success: false, error: 'Taker not initialized' };
      }
      ...
      console.log(`🚀 [${swapId}] Starting ${protocolName} coinswap: ${amount} sats, ${makerCount} makers`);
      ...
      const config = {
        ...
        zmqAddr: api1State.storedTakerConfig?.zmqAddr || 'tcp://127.0.0.1:28332',
        password: password || '',
        protocol: protocol,
      };
...
function registerSwapReportsHandlers() {
  ...
  ipcMain.handle('swapReports:get', async (event, swapId) => {
    try {
      ...
      const files = fs.readdirSync(reportsDir);
      const matchingFile = files.find(f => f.startsWith(swapId) && f.endsWith('.json'));
